{-# LANGUAGE PatternGuards, PackageImports, RankNTypes #-}
...
-- 
-- dsf: This is a copy of JSON.Generic from json-0.4.4 with support
-- added for Data.Text.
module JSON.Generic2
    ( toJSON
    , fromJSON
    , encodeJSON
    , decodeJSON
...
toJSON = (toJSON_generic toJSON)
         ...
         `extQ` (showJSON :: T T.Text)
  where
        -- Lists are simply coded as arrays.
        jList vs = JSArray $ map toJSON vs


toJSON_generic :: (Data a) => (forall b. Data b => b -> JSValue) -> a -> JSValue
toJSON_generic toJSON_base = generic
  where
        ...
        generic a =
            case dataTypeRep (dataTypeOf a) of
                AlgRep []  -> JSNull
                AlgRep [c] -> encodeArgs c (gmapQ toJSON_base a)
                AlgRep _   -> encodeConstr (toConstr a) (gmapQ toJSON_base a)
...
fromJSON j = fromJSON_generic fromJSON j
             `ext1R` jList
             --
...
fromJSON_generic :: (Data a) => (forall b. Data b => JSValue -> Result b) -> JSValue -> Result a
fromJSON_generic fromJSON_base j = generic
  where
        ...
        getConstr _ c = Error $ "fromJSON: bad constructor encoding " ++ show c ++ ": " ++ show j
        readConstr' t s = 
        ...
        construct c = evalStateT $ fromConstrM f c
          where f :: (Data a) => StateT [JSValue] Result a
                f = do js <- get; case js of [] -> lift $ Error "construct: empty list"; j' : js' -> do put js'; lift $ fromJSON_base j'
